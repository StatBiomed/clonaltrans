...
class CloneTranModel(nn.Module):
    ...
    def compute_obs_loss(self, y_pred, epoch, thres):
        ...
        for i in range(1, self.N.shape[0]):
            if epoch > thres:
                ...
                loss_obs += GaussianNLL(self.N[i], y_pred[i], torch.square(std_reshape[i] * self.scaling_factor[i]))

            else:
                loss_obs += PoissonNLL(y_pred[i] / self.scaling_factor[i], self.N[i] / self.scaling_factor[i])
    ...
    def compute_loss(self, y_pred, epoch, pbar):
        ...
        descrip = pbar_tb_description(
            ['L/K2Pro', 'L/Upper', 'L/Pop0', 'L/VarUb', 'L/L3', 'L/L10', 'L/L17', 'Model/lr'],
            [
